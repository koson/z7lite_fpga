...
parameter       H_TOTAL = 525 - 1  ;//一行总共需要计数的值
parameter       H_SYNC = 41 - 1    ;//行同步计数值
parameter       H_START = 43 - 1   ;//行图像数据有效开始计数值
parameter       H_END = 523 - 1    ;//行图像数据有效结束计数值
parameter       V_TOTAL = 286 - 1  ;//场总共需要计数的值
parameter       V_SYNC = 10 - 1    ;//场同步计数值
parameter       V_START = 12 - 1   ;//场图像数据有效开始计数值
parameter       V_END = 284 - 1    ;//场图像数据有效结束计数值
parameter       SQUARE_X    =   150;//方块的宽度
parameter       SQUARE_Y    =   150;//方块的长度
...
assign  vpg_de = (cnt_h >= H_START) && (cnt_h < H_END) && (cnt_v >= V_START) && (cnt_v < V_END);
